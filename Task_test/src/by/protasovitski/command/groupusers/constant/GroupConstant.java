package by.protasovitski.command.groupusers.constant;

public class GroupConstant {
    public final static String NEW_NAME_TASK = "nNameTask";
    public final static  String NEW_TASK_STATUS = "nstatus";
    public final static String TASK_ID = "id";

    public final static String ERROR_MESSAGE = "errorMessage";
    public final static String ERROR_MESSAGE_TEXT = "Заполните все поля";
    public final static String LISTGROUP ="group";
    public final static String TASK="task";
}
